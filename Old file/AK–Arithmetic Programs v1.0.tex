...
\Large Version 17.0: Collapse-Based Calibration for Weil RH over Finite Fields and Fermat's Last Theorem}
...
More precisely, for a fixed degree \(i\) and window \([0,\tau]\), any
...
On any fixed UCC window \([0,\tau]\) where the hypotheses of
...
The validation of a terrain cell $W \subset M_{\mathrm{Weil}}$ proceeds via the standard Gate Cascade (Chapter~1/9), specialized for spectral data.

\begin{specification}[B-Gate$^+$ for Weil]
On a window $W$ (e.g., a family of curves $C_\lambda$ over $\mathbb{F}_q$), the gate checks:
\begin{enumerate}
    \item \textbf{Persistence Check:} Are the Betti numbers $b_i$ constant on $W$? (No bifurcation).
    \item \textbf{Spectral Check:} Is $\Phi^{\mathrm{Weil}}_\tau(x) \approx 0$ for all sampled $x \in W$?
    \item \textbf{Budget Check:} Is the deviation attributable to the logged $\delta^{\mathrm{disc}}$ (e.g., floating-point precision of eigenvalue solver)?
\end{enumerate}
If passed, the AK Core issues a certificate: ``$W \subset Z_{\mathrm{Valid}}$''.
\end{specification}
...
Having defined the defect potential $\Phi^{\mathrm{Weil}}_\tau$ in Chapter~\ref{ch:weil-realization}, we now specify the acceptance criteria---the \emph{Spectral UCC}---that the AK Core uses to certify a terrain cell $W \subset M_{\mathrm{Weil}}$ as \emph{Valid}.
...
For a computation on a window $W \subset M_{\mathrm{Weil}}$:
...
\textbf{Pass Condition:} B-Gate$^+$ accepts the point $\theta$ if
\[
...
We expect $Z_{\mathrm{Valid}}$ to coincide with the locus of modular elliptic curves, while the ``hypothetical'' part of $M_{\mathrm{FLT}}$ corresponding to Frey curves of putative solutions is mapped to $Z_{\mathrm{Sing}}$ (or excluded entirely by B-Gate$^+$).
\end{remark}
...
\begin{enumerate}
    ...
    The local B-Gate$^+$ checks pass: $\Phi_{\mathrm{local}} \approx 0$.
...
\begin{itemize}
    ...
    \item For every hypothetical point $\theta$, the B-Gate$^+$ (Modularity Check) returns \texttt{FAIL: Type~IV Obstruction}.
...
\[
  ...
  \bigl\{ \theta \in M_{\mathrm{FLT}}
    \mid \text{\emph{B-Gate}}^{+}(\theta) = \mathsf{PASS} \bigr\}.
\]
...
[ATTEMPT k] Proposing hypothetical parameter theta_k in L_Frey...
         ...
         > B-Gate Status: BLOCKED (Type IV Obstruction).
         > Action: REJECT.
