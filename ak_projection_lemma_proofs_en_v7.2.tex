...
\subsection*{Z.9 Collapse Lemma and Structural Regularity Theorem (補強C)}
\addcontentsline{toc}{subsection}{Z.9 Collapse Lemma and Structural Regularity Theorem}

To complete the formal structure of collapse logic, we now present the central lemma  
which connects persistent topology, spectral energy decay, and Ext-class vanishing  
to analytic regularity in the context of AK-HDPST.

---

\begin{lemma}[Collapse Lemma]
\label{lem:collapse}
Let \( \mathcal{F}_t \in \mathsf{Filt} \) be a time-indexed family of derived filtration objects.  
Suppose the following three conditions hold:

\begin{enumerate}
    \item[(i)] \textbf{Topological collapse:}  
    \[
    \forall t \in T, \quad \mathrm{PH}_1(\mathcal{F}_t) = 0
    \]
    \item[(ii)] \textbf{Spectral energy decay:}  
    \[
    \lim_{k \to \infty} \left\| \widehat{u}(k, t) \right\|_{L^2} = 0 \quad \text{(dyadic shell average)}
    \]
    \item[(iii)] \textbf{Ext-class vanishing:}  
    \[
    \mathrm{Ext}^1(\mathcal{F}_t, \mathcal{G}) = 0 \quad \text{for all test objects } \mathcal{G} \in D^b(\mathcal{X})
    \]
\end{enumerate}

Then there exists a colimit object \( \mathcal{F}_0 := \colim_{t \to 0} \mathcal{F}_t \)  
...
\textbf{(i)} Persistent homology collapse implies the triviality of topological loops and components  
via functorial contraction (Appendix B, J.6).  

\textbf{(ii)} Spectral decay ensures that all high-frequency modes vanish, implying  
\( u(t) \in H^s(\mathbb{R}^3) \) for all \( s \), hence \( C^\infty \) regularity by Sobolev embedding (Appendix Z.3).

\textbf{(iii)} Ext$^1$-vanishing eliminates local-to-global obstruction, allowing sheaf colimit glueing  
to recover a global smooth object \( \mathcal{F}_0 \), interpreted as the regular solution \( u(t) \).

Each component aligns with Axioms A1–A7 and categorical collapse diagrams in Appendix J.8 and Z.4.
...
\paragraph{Interpretation.}
This lemma unifies topological, spectral, and categorical collapse mechanisms  
into a singular formal pathway toward global regularity.  
It serves as the foundational collapse theorem within AK theory—analogous in scope to the Beale–Kato–Majda criterion,  
but derived from structural invariants rather than pointwise PDE estimates.
...
\[
\text{All gluing obstructions vanish (e.g., \( \Sha(E) = 0 \)), and special function realization is enabled (e.g., \( K^{\mathrm{ab}} = K(\Theta_K(P)) \))}
\]
