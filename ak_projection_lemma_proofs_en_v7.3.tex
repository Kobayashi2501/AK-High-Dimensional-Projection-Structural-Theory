...
\subsection*{O.3 Collapse Functor Equivalence}

Let
\[
\mathcal{F}_{\text{Trop}}, \quad \mathcal{F}_{\text{Mirror}}, \quad \mathcal{F}_{\text{AbVar}}
\]
denote derived functors acting on the respective projection spaces:

\begin{align*}
\mathcal{F}_{\text{Trop}} &: \mathrm{Trop}(X) \to \mathcal{D}^b_{\text{triv}} \\
\mathcal{F}_{\text{Mirror}} &: \check{X} \to \mathcal{D}^b_{\text{flat}} \\
\mathcal{F}_{\text{AbVar}} &: J(X) \to \mathcal{D}^b_{\text{split}}
\end{align*}

\begin{theorem}[Triple Collapse Equivalence]
There exists a functorial collapse isomorphism:
\[
\mathcal{F}_{\text{Trop}}(X)
\simeq \mathcal{F}_{\text{Mirror}}(\check{X})
\simeq \mathcal{F}_{\text{AbVar}}(J(X))
\quad \Rightarrow \quad u(t) \in C^\infty
\]
\end{theorem}

\begin{proof}[Sketch]
Each functor maps to a derived category characterized by trivial barcode persistence and Ext$^1$-vanishing.  
Tropical collapse induces contractibility; mirror collapse flattens special Lagrangian fibers; Abelian collapse eliminates extension obstructions.  
Hence all three yield a smooth global solution.
\end{proof}
...
\subsection*{Z.10 Collapse Failure and Non-Collapsible Space}
\addcontentsline{toc}{subsection}{Z.10 Collapse Failure and Non-Collapsible Space}

We now formalize the notion of \textbf{collapse failure}—cases where the standard AK collapse sequence does not complete due to persistent structural obstructions.  
Such regions are of particular interest in both analytic PDEs and number-theoretic contexts (e.g., chaotic Navier–Stokes flow, nontrivial Selmer components).
