...
Let \( \mathcal{C}_{\text{Trop}}, \mathcal{C}_{\text{Mirror}}, \mathcal{C}_{\text{Jac}} \) denote collapse functors induced respectively by tropical, SYZ, and Jacobian projections.  
Then the composite functor:
\[
\mathcal{C}_{\text{Unified}} := \mathcal{C}_{\text{Trop}} \sqcup \mathcal{C}_{\text{Mirror}} \sqcup \mathcal{C}_{\text{Jac}} : \mathcal{D}^{b}_{\text{geom}}(X) \longrightarrow \mathcal{C}^\infty
\]
is a well-defined typed functor under the conditions of Z.12.2–Z.12.5, and induces analytic smoothness if the barcode vanishes:
\[
\mathrm{PH}_1(X) = 0 \Rightarrow \mathcal{C}_{\text{Unified}}(\mathcal{F}_X) \in \mathcal{C}^\infty.
\]
